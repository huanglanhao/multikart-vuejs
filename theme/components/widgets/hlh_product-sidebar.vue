  ...
  <div>
    ...
    <div class="collection-filter" :class="{'openFilterbar' : filter}">
          ...
          <div class="collection-collapse-block-content">
            <div class="collection-brand-filter">
              <ul class="category-list">
                <li>
                  <nuxt-link :to="{ path: '/collection/xz_leftsidebar/all'}">全部商品</nuxt-link>
                </li>
                <li v-for="(item,index) in categoryList" :key="index">
                  <nuxt-link :to="{ path: '/collection/xz_leftsidebar/all'}" @click="getCategoryFilter(category)">
                    {{item.categoryName}}</nuxt-link>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="product-service">
          <div class="media">
            ...
            <div class="media-body">
              <h4>免费送货</h4>
              <p style="font-size: 12px;line-height:25px">全球免费航运</p>
            </div>
          </div>
          <div class="media">
            ...
            <div class="media-body">
              <h4>关于退换货</h4>
              <p style="font-size: 12px;line-height:25px">所有宝贝付款后24小时内发货，支持7天无理由退换货</p>
            </div>
          </div>
          <div class="media">
            ...
            <div class="media-body">
              <h4>节日优惠</h4>
              <p style="font-size: 12px;line-height:25px">网上特别节日优惠</p>
            </div>
          </div>
          <div class="media border-0 m-0">
            ...
            <div class="media-body">
              <h4>线上支付</h4>
              <p style="font-size: 12px;line-height:25px">网银支付</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="theme-card">
        <h5 class="title-border">新品推荐</h5>
        <div class="offer-slider slide-1">

          <div v-swiper:mySwiper="swiperOption" style="height:70vh; overflow: auto">
                ...
                <div>
                  <div class="media" v-for="(item,index) in records" :key="index">
                    ...
                    <div class="media-body align-self-center">
                      ...
                      <h4 v-if="item.sale">
                        {{ discountedPrice(item) * currency.curr | currency(currency.symbol) }}
                        <del>{{ item.price * currency.curr | currency(currency.symbol) }}</del>
                      </h4>
                      <h4 v-else>{{ item.price * currency.curr | currency(currency.symbol) }}</h4>
                    </div>
                  </div>
                </div>
              </div>
              <!--<div class="swiper-slide" v-if="records('new products').length >= 4">
                <div>
                  <div class="media" v-for="(item, index) in records[10]" :key="index">
                    <nuxt-link :to="{ path: '/product/sidebar/'+item.id}">
                      <img class="img-fluid" :src="'http://img-test.gz-yami.com/' + records[{price}]" alt>
                    </nuxt-link>
                    <div class="media-body align-self-center">
                      <div class="rating">
                        <i class="fa fa-star"></i>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        categoryList: state => state.xz_home.records
      }),
      ...mapGetters({
    ...
    mounted() {
      this.getLastedProdPage({})

    },
    methods: {
      ...mapActions(["getLastedProdPage"]),
